...
interface FoodRunnerService {
    ...
    fun getMenuList(@Path( "restaurant_id") restaurantId :String) :Call<MenuModelResponse>

    @Headers("content-type:application/json", "token:ad314db5e5b1e2")
    @POST("v2/place_order/fetch_result")
    fun placeOrder(@Body placeOrderModel: PlaceOrderModel) :Call<PlaceOrderResponse>


    @Headers("content-type:application/json", "token:ad314db5e5b1e2")
    @GET("v2/orders/fetch_result/{user_id}")
    fun getPastOrders(@Path( "user_id") userId :String) :Call<PastOrdersModelResponse>
}
