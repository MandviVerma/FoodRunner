...
class SignUpActivity : AppCompatActivity() {
    ...
    private fun registerUser() {
       ...
       if( validate()) {
           ...
           else if (!et_email.text.toString().contains("@") && !et_email.text.toString().contains(".com")   )
               Toast.makeText(this, "Invalid Email",Toast.LENGTH_SHORT).show()
           else if (et_password.text.toString().length <4)
               Toast.makeText(this, "Password should be min. 4 characters",Toast.LENGTH_SHORT).show()
           else if (et_password.text.toString() != et_confirm_password.text.toString())
               Toast.makeText(this, "Password doesn't match",Toast.LENGTH_SHORT).show()
           else{

              val authModel = AuthModel(et_name.text.toString(),
                  et_mobile_number.text.toString(),et_password.text.toString(),
